...
export class StocksService {
  ...
  async getAll() {
    // Return persisted stocks enriched with latest score
  ...
  async getQuote(symbol: string) {
    ...
    await this.cache.set(cacheKey, result, 60); // 1 min
    return result;
  }
  ...
  async getFundamentals(symbol: string) {
    ...
    const result = {
      ...
      peTTM:           data?.peTTM,
      pbRatio:         data?.pbAnnual,
      psRatio:         data?.psTTM,
      evEbitda:        data?.currentEv_ebitdaTTM,
      roe:             data?.roeTTM,
      roa:             data?.roaTTM,
      revenueGrowth:   data?.revenueGrowthTTMYoy,
      epsGrowth:       data?.epsGrowthTTMYoy,
      grossMargin:     data?.grossMarginTTM,
      operatingMargin: data?.operatingMarginTTM,
      netMargin:       data?.netProfitMarginTTM,
      debtEquity:      data?.totalDebt_totalEquityAnnual,
      currentRatio:    data?.currentRatioAnnual,
      beta:            data?.beta,
  ...
  async getAnalyst(symbol: string) {
    const upper = symbol.toUpperCase();
    ...
    const result = {
      ...
      analystCount:   target?.lastUpdated,
