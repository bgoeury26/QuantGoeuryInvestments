...
export class FlowsService {
  ...
  private daysAgo(n: number) { const d = new Date(); d.setDate(d.getDate() - n); return d.toISOString().slice(0, 10); }
  ...
  async getInstitutionalHoldings(symbol: string) {
    ...
    try {
      const { data } = await axios.get(
        `https://efts.sec.gov/LATEST/search-index?q=%22${symbol}%22&dateRange=custom&startdt=${this.daysAgo(90)}&enddt=${this.daysAgo(0)}&forms=13F-HR`,
  ...
  async getInsiderTrades(symbol: string) {
    ...
    try {
      const { data } = await axios.get(
        `https://efts.sec.gov/LATEST/search-index?q=%22${symbol}%22&forms=4&dateRange=custom&startdt=${this.daysAgo(90)}&enddt=${this.daysAgo(0)}`,
        { headers: { "User-Agent": "QuantGoeuryInvestments contact@quant.com" } }
      );
  ...
  async getPoliticalTrades(symbol: string) {
    ...
    try {
      ...
      await this.cache.set("political", { symbol }, { trades, symbol }, 43200);
