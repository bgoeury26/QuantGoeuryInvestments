...
export class SentimentService {
  ...
  async getSentiment(symbol: string) {
    ...
    // Derive simple -1..1 score
    let score = 0;
    if (bullishPct != null && bearishPct != null) {
      score = (bullishPct - bearishPct);
    } else if (sentimentScore != null) {
      score = (sentimentScore - 0.5) * 2; // 0..1 → -1..1
    }
    ...
    const result = {
      ...
      buzz,
      weeklyAvgBuzz:   weeklyAvg,
      sectorAvgBullish: sectorAvg,
      articlesCount:   news?.totalResults ?? 0,
      articles,
      dataSource: [
        finnSent ? 'Finnhub' : null,
        news    ? 'NewsAPI' : null,
      ].filter(Boolean),
    };

    await this.cache.set(cacheKey, result, 900); // 15 min
    return result;
  }
  ...
  async getSocialVelocity(symbol: string) {
