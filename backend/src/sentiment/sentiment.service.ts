...
export class SentimentService {
  ...
  async getNewsSentiment(symbol: string) {
    ...
    try {
      ...
      const posWords = ["surge","gain","rise","beat","record","growth","strong","bullish","buy","upgrade"];
      const negWords = ["fall","drop","loss","miss","decline","weak","bearish","sell","downgrade","concern"];
      let score = 0, count = 0;
      ...
      const sentiment = count > 0 ? score / count : 0;
  ...
  async getRedditSentiment(symbol: string) {
    ...
    try {
      ...
      const posts = responses.flatMap((r: any) => r.data?.data?.children || []);
      const bullish = ["moon","buy","calls","bull","long","bullish","pump","squeeze"];
      const bearish = ["puts","short","bear","dump","sell","crash","bearish"];
      let score = 0, count = 0;
      ...
      const result = { posts: posts.slice(0, 20).map((p: any) => ({ title: p.data?.title, score: p.data?.score, subreddit: p.data?.subreddit })), totalMentions: posts.length, sentiment: count > 0 ? score / count : 0 };
