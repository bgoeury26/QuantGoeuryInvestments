    ...
    <AppShell title="Flows Intelligence">
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-4 mb-4">
        ...
        <div className="card p-5 lg:col-span-2">
          ...
          {isLoading ? <SkeletonCard rows={4} /> : flows?.institutional ? (
            <ResponsiveContainer width="100%" height={200}>
              <BarChart data={flows.institutional} margin={{ left: -10 }}>
                ...
                <Bar dataKey="netFlow" radius={[4, 4, 0, 0]}>
                  {flows.institutional.map((entry: {netFlow: number}, index: number) => (
                    <Cell key={index} fill={entry.netFlow >= 0 ? '#10b981' : '#ef4444'} />
                  ))}
                </Bar>
              </BarChart>
            </ResponsiveContainer>
          ) : <p className="text-xs text-muted text-center py-8">Configure SEC EDGAR / 13F API to load flows.</p>}
        </div>
        ...
        <div className="card p-5">
          ...
          ) : <p className="text-xs text-muted text-center py-8">No political trades data. Add Congress API key.</p>}
        </div>
      </div>
        ...
        {insiders?.length > 0 ? (
          <div className="overflow-x-auto">
            <table className="w-full">
              ...
              <tbody>
                {insiders.map((t: {symbol: string; insiderName: string; transactionType: string; shares: number; value: number; filingDate: string}, i: number) => (
                  <tr key={i} className="border-b border-border/30 hover:bg-surface-2/50">
                    ...
                    <td className="py-2.5 pr-4"><span className="text-xs text-text">{t.insiderName}</span></td>
                    ...
                    <td className="py-2.5"><span className="text-xs text-muted">{timeAgo(t.filingDate)}</span></td>
                  </tr>
                ))}
              </tbody>
            </table>
          </div>
        ) : <p className="text-xs text-muted text-center py-8">Configure Finnhub or FMP API key for insider data.</p>}
      </div>
    </AppShell>
  );
}
